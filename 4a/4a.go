...
func createMaterial(material Material, bag map[string]int, reactions []Reaction, oreCount *int) {

	fmt.Println("creating: ", material)
	// ore can be created anytime, free of charge => create some ;-)
	if material.name == "ORE" {
		bag["ORE"] = material.count + bag["ORE"]
		*oreCount += material.count
		return
	}
	...
	for r := 0; r< reactionCount; r++ {
		for _, m := range matchedReaction.input {
			if (bag[m.name] < m.count) {
				// not enough input materials available, create more!
				createMaterial(Material{m.name, m.count - bag[m.name]}, bag, reactions, oreCount)
				fmt.Println("Took ", m)
			}
			bag[m.name] = bag[m.name] - m.count
		}
		fmt.Println("Created ", matchedReaction.output.name, " ", matchedReaction.output.count)
		bag[matchedReaction.output.name] = bag[matchedReaction.output.name] + matchedReaction.output.count
	}
}
