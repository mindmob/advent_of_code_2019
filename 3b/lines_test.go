...
func Test_IsPointOnLine(t *testing.T) {

	line1 := NewLineWithCoordinate(5, 'R', 3, 3)
	res1 := IsPointOnLine(line1, coordinate{3, 3})
	assert.Equal(t, res1, true)
	res2 := IsPointOnLine(line1, coordinate{4, 3})
	assert.Equal(t, res2, true)
	res3 := IsPointOnLine(line1, coordinate{2, 3})
	assert.Equal(t, res3, false)
	res4 := IsPointOnLine(line1, coordinate{4, 4})
	assert.Equal(t, res4, false)

}

func Test_CountStepsToCoordinate(t *testing.T) {

	lines1 := ParseWire("R8,U5,L5,D3")
	d1 := CountStepsToCoordinate(lines1, coordinate{3, 3})
	assert.Equal(t, d1, 20)
	d2 := CountStepsToCoordinate(lines1, coordinate{6, 5})
	assert.Equal(t, d2, 15)

	lines2 := ParseWire("U7,R6,D4,L4")
	d3 := CountStepsToCoordinate(lines2, coordinate{3, 3})
	assert.Equal(t, d3, 20)
	d4 := CountStepsToCoordinate(lines2, coordinate{6, 5})
	assert.Equal(t, d4, 15)
	d5 := CountStepsToCoordinate(lines2, coordinate{18, 5})
	assert.Equal(t, d5, -1)
...
func Test_FindShortestIntersection(t *testing.T) {
	...
	lines1 := ParseWire("R8,U5,L5,D3")
	lines2 := ParseWire("U7,R6,D4,L4")
	res := FindShortestIntersection(lines1, lines2)
	assert.Equal(t, res, 30)

	lines1 = ParseWire("R75,D30,R83,U83,L12,D49,R71,U7,L72")
	lines2 = ParseWire("U62,R66,U55,R34,D71,R55,D58,R83")
	res = FindShortestIntersection(lines1, lines2)
	assert.Equal(t, res, 610)

	lines1 = ParseWire("R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51")
	lines2 = ParseWire("U98,R91,D20,R16,D67,R40,U7,R15,U6,R7")
	res = FindShortestIntersection(lines1, lines2)
	assert.Equal(t, res, 410)
